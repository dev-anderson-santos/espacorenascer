<div class="modal fade" id="modalGlobal" tabindex="-1" role="dialog">
    <!-- aria-hidden="true" -->
    <div class="modal-dialog modal-lg" role="document" style=" min-width: 50%;">
        <div class="modal-content" style="padding: 10px 5px 0 5px;">
            <div class="modal-header">
                <p class="modal-title" id="modalGlobalLabel" style="font-size: 18pt; color: black;">&zwnj;</p>
                <!-- [ESC] ou | Para fechar aperte [ESC] ou [Aqui] -->
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"  title="Fechar Modal" data-toggle="tooltip">
                    <span aria-hidden="true" style="font-size: 24pt; color: black;">&times;</span>
                    
                </button>
                <!-- tive que tirar o h4 por causa da criação de hieraquia no html - ases  - dev. FR -->
            </div>
                <div class="modal-body" id="getModalGlobalLoad" style="min-height: 20%; --overflow-y:scroll">
            </div>
        </div>
    </div>
</div><?php /**PATH C:\wamp64\www\espaco_juntos\resources\views/layouts/modalGlobal.blade.php ENDPATH**/ ?>