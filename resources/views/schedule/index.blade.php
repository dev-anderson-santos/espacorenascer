                            ...
                            <tr>
                                ...
                                @foreach ($rooms as $room)  
                                    ...
                                    <td style="text-align: center; {{ (!empty($schedule) && $schedule->tipo == 'Avulso') ? 'background:#17a2b8!important' : '' }}">
                                        @if (!empty($schedule))
                                            <a href="javascript:void(0)" style="{{ $schedule->tipo == 'Avulso' ? 'color:black!important' : '' }}" 
                                                onclick="modalGlobalOpen('{{ route('schedule.modal-schedule', ['room_id' => $room->id, 'hour_id' => $hour->id, 'user_id' => auth()->user()->id, 'data' => $_day]) }}', 'Agendamento')">
                                                {{ $schedule->user->name }}
                                            </a>
                                        @else
                                            <a href="javascript:void(0)" style="{{ (!empty($schedule) && $schedule->tipo == 'Avulso') ? 'color:black!important' : '' }}" onclick="modalGlobalOpen('{{ route('schedule.modal-schedule', ['room_id' => $room->id, 'hour_id' => $hour->id, 'user_id' => auth()->user()->id, 'data' => $_day]) }}', 'Agendamento')">
                                            @if (!empty($schedule))
                                                {{ $schedule->user->name }}
                                            @else
                                                Livre
